...
class LinearClassifier(nn.Module):
    def __init__(self, softness_num_classes , checkpoint_path, nn_model='resnet18', pretrained=True):
    ...
    def forward(self, rgb_input, tactile_input):
        ...
        return  self.softness_layer(combined_features)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
checkpoint_path = '/media/lab404/dataset2/AVTNet/runs/Dec04_21-35-30_lab404/checkpoint_0240.pth.tar'
print(f"Using device: {device}")
linear_classifier = LinearClassifier(softness_num_classes=2,  checkpoint_path=checkpoint_path, nn_model='resnet18', pretrained=True)
linear_classifier = linear_classifier.to(device)

batch_size = 192
num_workers = 16
use_wandb = False
dataset = ContrastiveLearningDataset(root_folder='/media/lab404/dataset2/AVTNet/touch_and_go')
train_dataset = dataset.get_dataset('tag_train', 2)
test_dataset = dataset.get_dataset('tag_test', 2,)
...
optimizer = torch.optim.Adam(linear_classifier.parameters(), lr=1e-4, weight_decay=1e-6)
...
for epoch in range(epochs):
    ...
    for counter, data in enumerate(pbar):
        rgb_image_q, _, stacked_gelsight_images_q, _, softness_label  = data
        ...
        softness_logits = linear_classifier(rgb_image_q, stacked_gelsight_images_q)
        ...
        pbar.set_description(
            f"Epoch {epoch}: Loss: {epoch_loss / (counter + 1):.2f}\tSoftness Train Accuracy: {top1_softness_train_accuracy.item() / (counter + 1):.2f}")
    scheduler.step()
    ...
    with torch.no_grad():
        for counter, data in enumerate(pbar):
            ...
            # update the progress bar message
            pbar.set_description(
                f"Epoch {epoch}:\tTrain Softness Accuracy: {top1_softness_train_accuracy.item():.2f}"
                f"\tTest Softness Accuracy: {top1_softness_accuracy.item() / (counter + 1):.2f}"
                )

    top1_softness_accuracy /= (len(test_loader))

    # save the model
    if top1_softness_accuracy > 74.9:
        torch.save(linear_classifier.state_dict(), f"runs/{subfolder}/linear_classifier_{epoch}_best_object_wise.pth")
    ...
    print(
        f"Epoch {epoch}:\tEpoch Loss: {epoch_loss:.2f}\tTrain Softness Accuracy: {top1_softness_train_accuracy.item():.2f}"
        f"\tTest Softness Accuracy: {top1_softness_accuracy.item():.2f}")
