...
##  path
parser.add_argument('--data_root', type=str, default='/media/lab404/code1/OS-AGDO-main/AGDDO15/', help='Path to the dataset directory')
parser.add_argument('--save_root', type=str, default='./saveAGDD015_models/', help='Path to save trained models')
##  image
...
if __name__ == '__main__':
    ...
    from data.agddo15_ego_orb import TrainData, TestData, SEEN_AFF, UNSEEN_AFF
    ...
    while True:
        for _, (img, ann, keypoints) in enumerate(TrainLoader):
            img, ann, keypoints = img.cuda(), ann.cuda().float(), keypoints.cuda()
            pred, loss_dict = model(img, label=ann, keypoints=keypoints)
            ...
            if (total_iter + 1) % args.eval_step == 0:
                ...
                for step, (image, gt_aff, object, mask_path, keypoints) in enumerate(tqdm(TestLoader)):
                    ...
                    ego_pred = model(image, keypoints, gt_aff=gt_aff)
